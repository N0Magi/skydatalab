...
export function getPropByPath(obj, path, strict) {
    ...
    // 子属性分割与AI组件返回的列名有冲突，这里修改为用#分割子属性
    path = path.replace(/^\./, '');
...
export default {
    ...
    created() {
        ...
        column.renderCell = function(h, data) {
            if (type !== 'selection' && _self.$scopedSlots.default) {
            ...
            return _self.showOverflowTooltip || _self.showTooltipWhenOverflow
                ? <div class="cell el-tooltip"
                    style={{ width: (data.column.realWidth || data.column.width) - 1 + 'px' }}>{renderCell(h, data)}</div>
